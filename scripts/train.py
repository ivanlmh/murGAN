...
for epoch in range(n_epochs):
    for i, (murga_samples, classical_samples) in enumerate(zip(murga_data_loader, classical_data_loader)):
        # Move to device
        ...
        if i % 10 == 0:
            ...
            # Tensorboard
            writer.add_scalar("Loss/D", d_loss.item(), global_step=epoch * len(murga_data_loader) + i)
            writer.add_scalar("Loss/G", g_loss.item(), global_step=epoch * len(murga_data_loader) + i)
            writer.add_scalar("Loss/D_real", d_real_loss.item(), global_step=epoch * len(murga_data_loader) + i)
            writer.add_scalar("Loss/D_fake", d_fake_loss.item(), global_step=epoch * len(murga_data_loader) + i)
            writer.add_scalar("Loss/D_domain", d_domain_loss.item(), global_step=epoch * len(murga_data_loader) + i)
            writer.add_scalar("Loss/G_fake", g_fake_loss.item(), global_step=epoch * len(murga_data_loader) + i)
            writer.add_scalar("Loss/G_domain", g_domain_loss.item(), global_step=epoch * len(murga_data_loader) + i)
            writer.add_scalar("Alpha", alpha, global_step=epoch * len(murga_data_loader) + i)
